...
| Aggregation | [`quantiles`][tusk.primitives.Quantiles] | — | ➕ | [`test_quantiles_default_values_are_hand_checkable`](https://github.com/Excidion/tusk/blob/main/tests/test_primitives_aggregation.py) | Unit test only, since featuretools has no `quantiles` counterpart to differential-test against. |
