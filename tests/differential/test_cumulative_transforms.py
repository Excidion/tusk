...
row shares one ``group_id``, and both sides run these primitives in
...
def test_cumulative_transforms_match_featuretools(name):
    ...
    theirs = featuretools_values(
        name, f"{name.upper()}(value) by group_id", grouped=True
    )
    assert all(
...
def test_cumulative_time_since_matches_featuretools_in_seconds(name):
    ...
    theirs = featuretools_values(
        name, f"{name.upper()}(occurred_at, flag) by group_id", grouped=True
    )
...
def test_cum_mean_divides_by_known_values_where_featuretools_divides_by_rows():
    ...
    theirs = featuretools_values(
        "cum_mean", "CUM_MEAN(value) by group_id", grouped=True
    )
...
def test_same_as_previous_does_not_forward_fill():
    ...
    theirs = featuretools_values(
        "same_as_previous", "SAME_AS_PREVIOUS(value) by group_id", grouped=True
    )
...
def test_absolute_diff_does_not_forward_fill():
    ...
    theirs = featuretools_values(
        "absolute_diff", "ABSOLUTE_DIFF(value) by group_id", grouped=True
    )
...
def test_percent_change_does_not_forward_fill():
    ...
    theirs = featuretools_values(
        "percent_change", "PERCENT_CHANGE(value) by group_id", grouped=True
    )
    assert [ours[0], ours[2], ours[4]] == [None, None, None]
    assert theirs[0] == 0.0
...
def test_cum_count_counts_known_values_where_featuretools_counts_rows():
    ...
    theirs = featuretools_values(
        "cum_count", "CUM_COUNT(label) by group_id", grouped=True
    )
